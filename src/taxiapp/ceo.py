...
def agent_summary(
    ...
    Näytetään ⚠️ viallisille agenteille.
    """
    return [
        {
            "name":     r.agent_name,
...
for module_path, class_name in agent_classes:
    try:
        ...
        try:
            ...
            # Korjataan automaattisesti iPhonen typografiset lainausmerkit
            parts = module_path.split(".")
            file_path = _sys.path[0] + "/" + "/".join(parts) + ".py"
            ...
            _sys.modules[module_path] = module
            logger.info(f"Agentti korjattu ja ladattu: {class_name}")
