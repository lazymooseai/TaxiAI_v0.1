...
Vaihe 5g — kaikki yhteen (Litteä kansiorakenne korjattu)
"""
...
if _missing:
    st.error(
        ...
        "Aseta ne Streamlit Cloud → App settings → Secrets"
    )
    st.stop()
...
with st.sidebar:
    ...
    with st.expander("📍 Aseta sijainti käsin", expanded=False):
        ...
        if st.button("Aseta sijainti", key="btn_set_loc"):
            # MUUTETTU TUONTI LITTEÄÄN KANSIORAKENTEESEEN
            try:
                from location import update_driver_location
                update_driver_location(float(mlat), float(mlon))
            except ImportError:
                st.warning("location.py -tiedostoa ei löydy samasta kansiosta.")
            st.session_state.pop("hotspot_cache", None)
            st.session_state.pop("hotspot_ts",    None)
            st.rerun()

    st.markdown("---")
    st.caption(f"v1.0 · {datetime.now(timezone.utc).strftime('%H:%M UTC')}")

# ── Välilehdet ────────────────────────────────────────────────
TABS = ["🏠 Kojelauta", "📅 Tapahtumat", "🔗 Linkit", "📊 Tilastot", "⚙️  Asetukset", "🔧 Ylläpito"]
tabs = st.tabs(TABS)
...
with tabs[1]:
    try:
        from events_tab import render_events_tab
